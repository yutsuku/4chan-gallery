...
if ( window.location.host != "boards.4chan.org" || window.location.pathname.indexOf("/thread/") == -1 ) {
	return;
}
...
Zepto(function($) {
	...
	var Gallery = {
		ImageExpansion_hook: ImageExpansion.expand,
		isShown: false,
		maxHeight: 0,
		lastHeight: 0,
		currentHeight: 0,
		...
		lastPage: 0,
		Keys: {
			"Previous": [33, 37, 38, 65, 87], // page up, left arrow, up arrow, a, w
		...
		InjectHTML: function() {
			var html = '<div id="gallery" class="fullscreen hidden"><div class="menu"><div class="contanyan"><div class="page-counter"><span class="current">0</span><span class="separator">/</span><span class="total">0</span></div><div class="close">x</div></div></div><div class="carousel"></div></div>';
			$(document.body).append(html);
		},
		SetOnDOMReady: function() {
			// prepare other elements that require DOM
			Gallery["wrapper"] 		= $("#gallery")[0];
			Gallery["items"]["dom"] = $(".carousel-inner");
			Gallery["lastPage"] 	= Gallery["items"]["dom"].length;
			Gallery["currentPages"] = $(".page-counter .current")[0];
			Gallery["totalPages"] 	= $(".page-counter .total")[0];
			Gallery["itemHeight"] 	= Gallery["wrapper"].clientHeight;
			Gallery["maxHeight"] 	= Math.max(Gallery["wrapper"].clientHeight, Gallery["wrapper"].scrollHeight, Gallery["wrapper"].offsetHeight);
			
			$(Gallery["currentPages"])[0].innerHTML = Gallery["currentPage"];
			$(Gallery["totalPages"])[0].innerHTML 	= Gallery["lastPage"];
		},
		AddEventListeners: function() {
			$("#gallery").on("scroll", function(event) {
				// skip events with same scroll position
				if ( this.scrollTop == Gallery.lastHeight ) return;
				
				Gallery.lastHeight = this.scrollTop;
				Gallery.currentHeight = this.scrollTop;
				
				var percent = Gallery.currentHeight / Gallery["itemHeight"] * 100; // 0 - first page, 100 - second page, 200 - third page and so on
				var newPage = 1 + Math.round(percent / (100 * Gallery["lastPage"]) * Gallery["lastPage"]);

				
				if ( newPage != Gallery["currentPage"] ) {
					Gallery["currentPage"] = newPage;
					$(Gallery["currentPages"])[0].innerHTML = newPage;
				}
			...
			$(document).on("keydown", function(event) {
				...
				if ( event.Hotkey() == "Previous" ) {
					Gallery.Previous();
					event.preventDefault();
				} else if ( event.Hotkey() == "Next" ) {
					Gallery.Next();
					event.preventDefault();
				} else if ( event.Hotkey() == "Close") {
					Gallery.Close("hotkey");
				}
			});
			...
			$(document).on("click", function(event) {
				...
				// close overlay if clicked anywhere but on media element
				if (	!$(event.target).closest(".item img").length &&
						!$(event.target).closest(".file .fileThumb img").length // ignore clicks from thumbnails in thread
				) {
					Gallery.Close("mouse target");
				};
			});
		},
		InjectData: function() {
			...
			$("#gallery .carousel").append(template_data);
		},
		// recalculate containers heights
		SetHeights: function() {
			Gallery["itemHeight"] 	= Gallery["wrapper"].clientHeight;
			Gallery["maxHeight"] 	= Math.max(Gallery["wrapper"].clientHeight, Gallery["wrapper"].scrollHeight, Gallery["wrapper"].offsetHeight);
		},
	...
	// Check code key agaisnt our defined hotkeys
	KeyboardEvent.prototype.Hotkey = function() {
		for (var key in Gallery.Keys) {
			if (Gallery.Keys.hasOwnProperty(key)) {
				if ( Gallery.Keys[key].indexOf(this.keyCode) >= 0 ) {
					if ( this.ctrlKey == true || this.shiftKey == true || this.altKey == true || this.metaKey == true ) {
						return false;
					}
					return key;
				}
